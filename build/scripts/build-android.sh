...
mv musicalc.apk "$DIST_DIR/musicalc-unaligned.apk"
"$ZIPALIGN_PATH" -f -v -P 16 4 "$DIST_DIR/musicalc-unaligned.apk" "$DIST_DIR/musicalc.apk"

# Verify alignment
"$ZIPALIGN_PATH" -c -v -P 16 4 "$DIST_DIR/musicalc.apk"
...
rm tmp_lib.so
echo "Build and Verification Complete: build/dist/musicalc.apk"
